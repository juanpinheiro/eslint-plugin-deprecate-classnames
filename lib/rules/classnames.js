...
const { extractClassNamesFromClassName } = require('../utils/extractClassNames');

module.exports = {
  meta: {
    docs: {
      description: 'Disallow usage of deprecated classNames in "className" attribute',
    ...
    return {
      JSXAttribute(node) {
        if (node.name.name === 'className') {
          const classNames = extractClassNamesFromClassName(node);
